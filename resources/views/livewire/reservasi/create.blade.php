<div>
    <x-button wire:click="$set('open', true)" class="bg-green-600 hover:bg-green-700">Tambah Reservasi</x-button>

    <x-dialog-modal wire:model="open">
        <x-slot name="title">Tambah Reservasi</x-slot>
        <x-slot name="content">
                ...
                <div>
                    ...
                    <select wire:model.defer="villa_pricing_id" class="w-full border-gray-300 rounded-md shadow-sm">
                        ...
                        @foreach ($pricings as $pricing)
                            <option value="{{ $pricing->id_villa_pricing }}">ID #{{ $pricing->id_villa_pricing }}
                            </option>
                        @endforeach
                    </select>
