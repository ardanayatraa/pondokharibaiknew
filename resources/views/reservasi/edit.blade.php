{{-- resources/views/reservasi/edit.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-sm text-gray-800">Edit Reservasi</h2>
    </x-slot>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div>
                            ...
                            <select id="guest_id" name="guest_id"
                                ...
                                @foreach ($guests as $g)
                                    <option value="{{ $g->id_guest }}" @selected(old('guest_id', $reservasi->guest_id) == $g->id_guest)>{{ $g->full_name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('guest_id')
                        ...
                        <div>
                            ...
                            <select id="villa_id" name="villa_id"
                                ...
                                @foreach ($villas as $v)
                                    <option value="{{ $v->id_villa }}" @selected(old('villa_id', $reservasi->villa_id) == $v->id_villa)>{{ $v->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('villa_id')
                        ...
                        <div>
                            ...
                            <select id="villa_pricing_id" name="villa_pricing_id"
                                ...
                                @foreach ($villaPricings as $p)
                                    <option value="{{ $p->id_villa_pricing }}" @selected(old('villa_pricing_id', $reservasi->villa_pricing_id) == $p->id_villa_pricing)>
                        ...
                        <div>
                            ...
                            <x-input id="start_date" name="start_date" type="date" class="mt-1 block w-full"
                                :value="old('start_date', $reservasi->start_date)" required />
                            @error('start_date')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
                        <div>
                            ...
                            <x-input id="end_date" name="end_date" type="date" class="mt-1 block w-full"
                                :value="old('end_date', $reservasi->end_date)" required />
                            @error('end_date')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <x-input id="total_amount" name="total_amount" type="number" class="mt-1 block w-full"
                                :value="old('total_amount', $reservasi->total_amount)" required />
                            @error('total_amount')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
