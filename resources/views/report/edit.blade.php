...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-sm text-gray-800">Edit Report</h2>
    </x-slot>
    <div class="py-2">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <x-input id="report_type" name="report_type" type="text" class="mt-1 block w-full"
                                :value="old('report_type', $report->report_type)" required autofocus />
                            @error('report_type')
                        ...
                        <div>
                            ...
                            <x-input id="date_range" name="date_range" type="text" class="mt-1 block w-full"
                                :value="old('date_range', $report->date_range)" required />
                            @error('date_range')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
