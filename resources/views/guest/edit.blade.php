<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-sm text-gray-800">Edit Guest</h2>
    </x-slot>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div>
                            ...
                            <x-input id="username" name="username" type="text" class="mt-1 block w-full"
                                :value="old('username', $guest->username)" required autofocus />
                            @error('username')
                        ...
                        <div class="md:col-span-3">
                            ...
                            <textarea id="address" name="address" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" rows="2">{{ old('address', $guest->address) }}</textarea>
                            @error('address')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <x-input id="passport_number" name="passport_number" type="text"
                                class="mt-1 block w-full" :value="old('passport_number', $guest->passport_number)" />
                        ...
                        <div>
                            ...
                            <x-input id="birthdate" name="birthdate" type="date" class="mt-1 block w-full"
                                :value="old('birthdate', $guest->birthdate)" />
                            @error('birthdate')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select id="gender" name="gender"
                                ...
                                <option value="male" @selected(old('gender', $guest->gender) == 'male')>Laki-laki</option>
                                <option value="female" @selected(old('gender', $guest->gender) == 'female')>Perempuan</option>
                            </select>
                            @error('gender')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                            @enderror
