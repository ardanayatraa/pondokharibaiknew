                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="villa_id" name="villa_id"
                                ...
                                @foreach ($villas as $v)
                                    <option value="{{ $v->id_villa }}" @selected(old('villa_id', $villa_pricing->villa_id) == $v->id_villa)>{{ $v->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('villa_id')
                        ...
                        @foreach (['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'] as $day)
                            <div>
                                ...
                                <x-input id="{{ $day }}_pricing" name="{{ $day }}_pricing"
                                    type="number" step="0.01" class="mt-1 block w-full" :value="old($day . '_pricing', $villa_pricing->{$day . '_pricing'})"
                                    required />
                                @error($day . '_pricing')
                                    <span class="text-sm text-red-600">{{ $message }}</span>
                                @enderror
